...
import { getTechnology } from "../graphql/queries";
import { updateTechnology } from "../graphql/mutations";
const client = generateClient();
export default function TechnologyUpdateForm(props) {
  const {
    id: idProp,
    technology: technologyModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = technologyRecord
      ? { ...initialValues, ...technologyRecord }
      : initialValues;
  ...
  const [technologyRecord, setTechnologyRecord] =
    React.useState(technologyModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getTechnology.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getTechnology
        : technologyModelProp;
    ...
    queryData();
  }, [idProp, technologyModelProp]);
  React.useEffect(resetStateValues, [technologyRecord]);
  const validations = {
    Name: [],
    Description: [],
    image: [{ type: "URL" }],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateTechnology.replaceAll("__typename", ""),
            variables: {
              input: {
      ...
      {...getOverrideProps(overrides, "TechnologyUpdateForm")}
      {...rest}
    >
      <TextField
        label="Name"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || technologyModelProp)}
