...
import { updateUserTL } from "../graphql/mutations";
const client = generateClient();
export default function UserTLUpdateForm(props) {
  const {
    id: idProp,
    userTL: userTLModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = userTLRecord
      ? { ...initialValues, ...userTLRecord }
      : initialValues;
  ...
  const [userTLRecord, setUserTLRecord] = React.useState(userTLModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getUserTL.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getUserTL
        : userTLModelProp;
      setUserTLRecord(record);
    };
    queryData();
  }, [idProp, userTLModelProp]);
  React.useEffect(resetStateValues, [userTLRecord]);
  const validations = {
    username: [],
    email: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateUserTL.replaceAll("__typename", ""),
            variables: {
              input: {
                id: userTLRecord.id,
                ...modelFields,
      ...
      {...getOverrideProps(overrides, "UserTLUpdateForm")}
      {...rest}
    >
      <TextField
        label="Username"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || userTLModelProp)}
